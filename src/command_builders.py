...
def build_postgres_smoke_client_command(settings: Dict[str, Any]) -> str:
    """Build Postgres smoke test client command."""
...
sleep {warmup_delay}
psql -h $SERVICE_HOSTNAME -p $SERVICE_PORT -U postgres -d {db_name} -c '{query}'"""


def build_postgres_stress_client_command(settings: Dict[str, Any]) -> str:
    """Build Postgres stress test client command."""
...
for i in $(seq 1 $NUM_SELECTS); do
  ...
  QUERY_TYPE=$((i % 4))
  case $QUERY_TYPE in
    0) psql -h $SERVICE_HOSTNAME -p $SERVICE_PORT -U postgres -d $DB_NAME -c "SELECT COUNT(*) FROM $TABLE_NAME WHERE value = $((i % 1000));" > /dev/null 2>&1 ;;
    1) psql -h $SERVICE_HOSTNAME -p $SERVICE_PORT -U postgres -d $DB_NAME -c "SELECT COUNT(*) FROM $TABLE_NAME WHERE value BETWEEN $((i % 500)) AND $((i % 500 + 100));" > /dev/null 2>&1 ;;
    2) psql -h $SERVICE_HOSTNAME -p $SERVICE_PORT -U postgres -d $DB_NAME -c "SELECT COUNT(*) FROM $TABLE_NAME WHERE payload LIKE '%record_%';" > /dev/null 2>&1 ;;
    3) psql -h $SERVICE_HOSTNAME -p $SERVICE_PORT -U postgres -d $DB_NAME -c "SELECT value, COUNT(*) FROM $TABLE_NAME GROUP BY value ORDER BY COUNT(*) DESC LIMIT 10;" > /dev/null 2>&1 ;;
  esac
done
...
def build_chroma_stress_client_command(settings: Dict[str, Any]) -> str:
    """Build ChromaDB stress test client command using curl (no Python deps)."""
    ...
    # Use curl-based approach since chromadb Python package may not be available
    return f"""sleep {warmup_delay}

echo "=== ChromaDB Stress Test ==="
...
curl -s -X POST "$SERVICE_URL/api/v1/collections" \\
  -H "Content-Type: application/json" \\
  -d '{{"name": "stress_test", "get_or_create": true}}' > /dev/null

# Get collection ID
COLLECTION_ID=$(curl -s "$SERVICE_URL/api/v1/collections/stress_test" | grep -o '"id":"[^"]*"' | head -1 | cut -d'"' -f4)
...
for i in $(seq 1 {num_vectors}); do
  # Generate random embedding (simplified - just random floats)
  EMBEDDING=$(python3 -c "import random; print([random.random() for _ in range({dim})])")
  
  curl -s -X POST "$SERVICE_URL/api/v1/collections/$COLLECTION_ID/add" \\
    -H "Content-Type: application/json" \\
    -d "{{\\"ids\\": [\\"id_$i\\"], \\"embeddings\\": [$EMBEDDING]}}" > /dev/null
...
for i in $(seq 1 {num_queries}); do
  ...
  curl -s -X POST "$SERVICE_URL/api/v1/collections/$COLLECTION_ID/query" \\
    -H "Content-Type: application/json" \\
    -d "{{\\"query_embeddings\\": [$QUERY_VEC], \\"n_results\\": {top_k}}}" > /dev/null
...
def build_ollama_smoke_client_command(settings: Dict[str, Any]) -> str:
    """Build Ollama smoke test client command."""
    model = settings.get("model", "llama2")
    num_requests = settings.get("num_requests", 5)
    max_retries = settings.get("max_retries", 30)
    
    return f"""echo "Checking Ollama service at $SERVICE_URL"
echo ""
...
def get_default_env(service_type: str, settings: Dict[str, Any]) -> Optional[Dict[str, str]]:
...
def validate_settings(settings: Dict[str, Any], context: str = "") -> None:
    ...
    positive_fields = [
        "num_inserts", "num_selects", "num_vectors", "num_queries",
        "num_requests", "warmup_delay", "warmup_seconds", "max_retries",
        "top_k", "dim", "max_tokens", "tensor_parallel_size"
    ]
    ...
    for field in positive_fields:
        if field in settings:
            ...
            if field in ["num_inserts", "num_selects", "num_vectors", "num_queries", "num_requests"]:
                if value < 1:
                    raise ValueError(
