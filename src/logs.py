...
class LogManager:
    ...
    def __init__(
        self,
        communicator: Optional[SSHCommunicator] = None,
    ...
    def list_remote_logs(
        self, benchmark_id: str, working_dir: str = "~"
    ) -> List[LogFile]:
        """
            ...
            if len(parts) >= 2:
                try:
                    ...
                    if match:
                        ...
                        job_type = (
                            "service"
                            if not job_name.startswith("client-")
                            else "client"
                        )
    ...
    def search_logs(
        self, benchmark_id: str, pattern: str, case_insensitive: bool = True
    ) -> List[Tuple[str, str]]:
        """
...
def format_log_list(logs: List[LogFile]) -> str:
    ...
    lines = [f"{'Name':<45} {'Size':<10} {'Type':<10} {'Job ID'}", "-" * 80]
