    ...
    for attempt in range(retries):
        ...
        if result.success:
            ...
            if status_code == "000":
                ...
                simple_cmd = (
                    f"curl -s --max-time {timeout} {url} > /dev/null 2>&1 && echo OK"
                )
        ...
        if attempt < retries - 1:
            print(
                f"  Health check attempt {attempt + 1}/{retries} failed, retrying in {retry_delay}s..."
            )
...
def check_port_open(
    communicator: SSHCommunicator, hostname: str, port: int, timeout: int = 5
) -> bool:
    """
